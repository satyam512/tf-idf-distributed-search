...
public class UserSearchHandler implements OnRequestCallback {
    ...
    private List<FrontendSearchResponse.SearchResultInfo> filterResults(SearchModel.Response searchClusterResponse,
        ...
        for (int i = 0; i < searchClusterResponse.getResultDocumentsCount() && i < maxResults; i++) {

            int normalizedDocumentScore = normalizeScore(searchClusterResponse.getResultDocuments(i).getScore(), maxScore);
            if (normalizedDocumentScore < minScore) {
                continue; // break in the lecture is an error
            }

            String documentName = searchClusterResponse.getResultDocuments(i).getDocumentName();
    ...
    private static double getMaxScore(SearchModel.Response searchClusterResponse) {
        if (searchClusterResponse.getResultDocumentsCount() == 0) {
            return 0;
        }
        return searchClusterResponse.getResultDocumentsList()
                .stream()
